...
let
  ...
  inherit (types) bool lines package listOf attrs attrsOf str nullOr;
...
in {
  ...
  config = 
  (let
    inherit (lib) mapAttrsFlatten filterAttrs;
    inherit (builtins) head tail match toJSON toUpper;
